...
def LSC(I: np.ndarray, nRows: int, nCols: int, superpixelnum: int, ratio: float, label: np.ndarray):
    ...
    if TEST_RGB2LAB:
        ...
        print(compare_2D_matrix(R.copy().reshape([nRows, nCols]).transpose([1, 0]), data["tR"]), end="", flush=True)
        print(compare_2D_matrix(G.copy().reshape([nRows, nCols]).transpose([1, 0]), data["tG"]), end="", flush=True)
        print(compare_2D_matrix(B.copy().reshape([nRows, nCols]).transpose([1, 0]), data["tB"]), end="", flush=True)
        print(compare_2D_matrix(L.copy().reshape([nRows, nCols]).transpose([1, 0]), data["tL"]), end="", flush=True)
        print(compare_2D_matrix(a.copy().reshape([nRows, nCols]).transpose([1, 0]), data["ta"]), end="", flush=True)
        print(compare_2D_matrix(b.copy().reshape([nRows, nCols]).transpose([1, 0]), data["tb"]), end="", flush=True)
        exit()
